...
4. **[IMPORTANT] Thread separators (`---`) between tweets in a thread.** Readwise Reader flattens a thread into one highlight's text with tweets separated by `\n\n` — but a single long tweet *also* uses `\n\n` between its own paragraphs, so the boundary is ambiguous from the flattened text. The importer must recognise per-tweet structure in the Reader `html_content` and insert an explicit separator during the reimport (rides with item 1). Until then, v0.5.2 renders thread tweets as spaced paragraphs (readable, but not visually separated as distinct tweets).
5. **Quoted/reply tweet blockquote styling.** Reader embeds the quoted tweet as plain lines (`Name` / `@handle` / text / `Posted …`) with no `> ` markup, so there is nothing for the blockquote renderer to catch. The importer must detect and mark the quoted-tweet block during the reimport (rides with item 1).
